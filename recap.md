...
- Choix de Tom :
  ...
  - saisie : le message du client collé en entier, des notes courtes en allemand et des abréviations internes. **Les vrais cas montrent aussi environ 5 % de notes en français** (et quelques-unes en italien ou en anglais).
...
1. **Nettoyage des messages collés.** Sont ignorés :
   ...
   - les e-mails, les téléphones (y compris « +41 (0)44 … »), les IBAN et numéros de carte, les dates, les numéros de 5 chiffres ou plus, les numéros de référence (« RE-2026/48213 » → « RE ») ;
   - les adresses (y compris « Seestr. 50 » et « PLZ + lieu »), **remplacées par « (Adresse) »**. Une note qui n'est qu'une adresse, fréquente pour 6102 Adressänderung, garde ainsi un indice. « 2024 Fotobuch » est une année, pas une PLZ ;
...
2. **Abréviations internes** (table `ABK`).
   - Liste devinée au départ : FB, AB, RE, GS, LW, Kd., erh., n. erh., Best., Rekla…
   - **Ajoutées d'après les vrais cas :** RG/RGs/Rg. (Rechnung), AT/ATs/AT-Nr. (Auftrag), DB (Doppelbestellung), GU (Gutschein), CC (Kreditkarte), PB (Fotobuch), CL (Kunde, notes en français).
   - Fréquences dans les notes : KD 237 fois, AT 51, RG 46, CAS 13 (n° de cas).
   - Une clé tout en majuscules est sensible à la casse, pour ne pas toucher « ab ». « RE: » n'est pas remplacé. Revers connu : « nach AT » (Autriche) devient « nach Auftrag ».
...
6. **Nouveau en v4 : « Fälle importieren und testen ».**
   ...
   - **Colonnes détectées automatiquement :** texte, code, ligne d'en-tête (même sous des lignes de titre), feuille, remplissage vers le bas pour les rapports groupés.
     ...
     - Une cellule de texte courte (8 mots au plus) qui n'est que le nom d'une classification, comme un titre « Adressänderung », est ignorée et comptée (`labelCells`). Le problème est apparu sur les vrais cas.
...
## Premiers vrais cas (24 septembre 2026)

Tom a envoyé un export réel du CRM : `Cases_Tom_Steinbach_6-months_2026-09-24.csv`. Il contient des données clients : **il ne doit jamais aller dans le dépôt** (le `.gitignore` exclut `*.csv`). Pour une nouvelle session, Tom doit le renvoyer.

**Format (Dynamics CRM) :**
- 798 lignes, 16 colonnes, séparées par « ; », en UTF-8 avec BOM.
- Colonnes utiles :
  - `Betreff` = la classification (« 50401 - Abfrage Auftragsstatus ») : 791 lignes reconnues, 117 codes différents ;
  - `Problem / Beschreibung` = le texte ;
  - `Anfragetitel` = le titre ;
  - `Erstellt am` = la date (« 13.06.2025 11:25:21 »).
- Autres colonnes :
  - à ne jamais prendre comme texte : `Kunde` (nom du client), `Anfrage-ID`, `Anfragenummer`, `Lösung` (écrite après coup) ;
  - `Ursprung` (Telefon 413, E-Mail 221, Kontaktformular 164) et `Anfragetyp` (Information, Buchhaltung, Support…).

**Ce que montrent les données :**
- **Les textes sont surtout des notes courtes de Tom**, pas des e-mails entiers. La médiane est d'environ 50 caractères au téléphone et par e-mail, et de 122 pour le formulaire de contact. Style : « KD wünscht … », « KD teilt mit … », « RG nicht erhalten », « DB aufgrund Systemfehler ».
- Environ 92 % des notes sont en allemand, 5 % en français (surtout les notes d'appels de Tom), et quelques-unes en italien ou en anglais.
- **Titre = classification :** dans 162 cas, le titre donne le même code que `Betreff`. Il gonflerait le test, donc il est ignoré (voir `labelCells`).
- **6102 Adressänderung (2e code le plus fréquent) :** la note est souvent juste l'adresse, par ex. « Neu: … », « alte Adresse: … » ou une rue et une PLZ seules.
- **Codes les plus fréquents :**

  | Code | Libellé | Cas |
  |---|---|---|
  | 50401 | Abfrage Auftragsstatus | 86 |
  | 50403 | Auftrag ändern/stornieren | 56 |
  | 6102 | Adressänderung | 55 |
  | 30208 | Projektgestaltung | 52 |
  | 50103 | Rechnungskorrektur | 46 |
  | 50105 | Rechnungsstatus | 36 |
  | 50702 | Doppelbestellung | 36 |
  | 20209 | Projektverwaltung | 28 |
  | 50502 | Lieferzeit | 24 |
  | 6251 | Rechnung ausstellen | 23 |

  51 codes n'apparaissent qu'une fois.

**Méthode de mesure honnête :**
- Découpage **par date** :
  - `dev` = les 2/3 les plus anciens (527 cas, du 29.01.2025 au 14.07.2026), pour améliorer les mots-clés ;
  - `test` = le 1/3 le plus récent (264 cas, du 15.07.2026 au 22.09.2026), **pour vérifier seulement, jamais pour ajuster**.
- Commande : `node tests/echte-faelle.js export.csv --split dev|test --cfg desc|both [--learned] [-v]`.
  - `desc` = description seule ;
  - `both` = titre, s'il n'est pas un libellé, et description.

**Résultats avant les nouvelles règles** (v4 avec abréviations réelles et « (Adresse) ») :

| Mesure | Platz 1 | Parmi les 3 | Kein Vorschlag |
|---|---|---|---|
| dev, description seule | 27 % | 39 % | 17 % |
| test, description seule | 36 % | 46 % | 15 % |
| dev, titre + description | 31 % | 44 % | 13 % |
| test, titre + description | 36 % | 50 % | 11 % |

Avant ces deux ajouts, les chiffres étaient d'environ 1 à 2 points plus bas.

**Apprentissage (« leave-one-out » sur les 791 cas, titre + description) :**

| Variante | Platz 1 | Parmi les 3 |
|---|---|---|
| Mots-clés seuls | 32 % | 45 % |
| Avec les cas appris (`gelernt`) | 47 % | 63 % |
| Meilleure variante (`top3_40`) | 48 % | 63 % |

- Avec la description seule, l'apprentissage fait passer de 40 % à 51 % parmi les 3.
- **Conclusion :** importer ses propres cas (« Alle übernehmen ») aide nettement.

**Principales causes d'erreurs (dev) :**
- mots-clés absents pour les tournures de Tom : Mahnstopp, Rabattcode nachtragen, Zahlung mitgeteilt, offene RG, Bilder hochladen/reinziehen… ;
- 6102 : note sans autre mot qu'une adresse ;
- notes en français ou en italien ;
- des codes voisins confondus :
  - 50401 ↔ 50410 (statut de commande / retard) ;
  - 50103 ↔ 50207 / 50112 (Rechnungskorrektur / Gutschein / Kulanz) ;
  - 20209 ↔ 20208 (projets) ;
...
5. **Régler l'apprentissage avec `methoden`** (variantes). Sur les cas de Tom, `top3_40` et `gelernt` (`{agg:"max", w:40}`, la variante par défaut) sont presque égales : pas de raison de changer pour l'instant.
6. **Si les mots-clés restent insuffisants :** une version avec IA serait bien plus fiable sur du texte libre, mais seulement avec l'accord de l'employeur (données clients).
...
Classification/
├── CLAUDE.md            (= ce récap)
├── recap.md             copie de ce récap (à joindre au début d'une nouvelle session)
...
└── tests/
    ...
    ├── echte-faelle.js  node tests/echte-faelle.js export.csv --split dev|test → mesure sur un export CRM, découpé par date
...
- **Données clients :**
  ...
  - l'envoi de vrais cas à une IA doit être autorisé par l'employeur. Tom a envoyé un export réel le 24.09.2026 : ne rien en copier dans le dépôt, et ne citer dans les réponses que des chiffres ou des tournures générales, jamais de noms ni de numéros.
